...
class TestProductRoutes(TestCase):
    ...
    def test_update_product_not_found(self):
        ...
        invalid_product_id = 0  # Assuming 0 is an invalid ID
        product_data = {
        ...
        response = self.client.put(f"{BASE_URL}/{invalid_product_id}", json=product_data)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        data = response.get_json()
        print("##########################", data["message"])
        self.assertIn("was not found", data["message"])
    ...
    def test_update_product_empty_id(self):
        ...
        empty_product_id = " "
        product_data = {
            "name": "Product with Empty ID",
            "description": "This product has an empty ID field",
            "price": 0.0,
            "category": "None"
        }

        response = self.client.put(f"{BASE_URL}/{empty_product_id}", json=product_data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        data = response.get_json()
        print("##########################", data["message"])
        self.assertIn("Invalid product ID", data["message"])
